...
def send_receive_client_message(client_connection, client_ip_addr):
    ...
    # Second error: need to encode messages as bytes
    if len(clients) < 2:
...
def update_client_names_display(name_list):
    ...
    # third error: need to decode bytes back to string for concatenation
    for c in name_list:
        tkDisplay.insert(tk.END, str(c)+"\n")
