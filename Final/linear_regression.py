...
class Predict:
    def __init__(self,root):
        ...
        self.img = ImageTk.PhotoImage(Image.open("/Users/hendy/Desktop/AI/AI_files/opencv-course/Section #3 - Faces/machine_learning.jpeg"))
    ...
    def apply_regression(self,file_name):
        ...
        Label(self.root, text = 'The accuracy of our lienar model = ' + str("{:.2f}".format(self.accuracy))).grid(row=2,column=0)
        Label(self.root, text = ' The reg coef of our lienar model = ' + str("{:.2f}".format(self.regression_coefficient[0][0]))).grid(row=3,column=0)
        Label(self.root, text = '       The intercep of our lienar model = ' + str("{:.2f}".format(self.y_intercept[0]))).grid(row=4,column=0)
        
        e  = Entry(self.root, width = 20,fg = "blue")
        e.grid(row=6,column=0)
        e.insert(0, 'Enter your area (Delete this!)') # default text
        def clicked():
            label = Label(self.root, text = "The predicted price = " + str("{:.2f}".format(reg.predict([[int(e.get())]])[0][0])))
            label.grid(row=8,column=0)

        my_button = Button(self.root, text = 'Predict price',command = clicked, fg = "blue") # no () after the function's name, fg, forground color, bg, background color
        my_button.grid(row=7, column=0)
        #Button(self.root,text='Exit',fg='red',command=self.root.destroy).grid(row=9,column=1)
